...
export function getDocContent(
  ...
  try {
    ...
    if (!slug || slug.length === 0) {
      ...
      filePath = path.join(contentDir, category, tool, version, ...slug) + '.md'
      if (!fs.existsSync(filePath)) {
        filePath = path.join(contentDir, category, tool, version, ...slug, 'index.md')
      }
    }
...
export function getAllTools(): { category: string; tool: string; versions: string[] }[] {
  ...
  for (const category of categories) {
    ...
    const toolDirs = fs.readdirSync(categoryDir).filter((f) =>
      fs.statSync(path.join(categoryDir, f)).isDirectory()
    )
    for (const tool of toolDirs) {
      const toolDir = path.join(categoryDir, tool)
      const versions = fs.readdirSync(toolDir).filter((f) =>
