...
The `skipCache` property is only available on the client-side. To access `getToken()` from the client-side, use the [`useSession()`](https://clerk.com/docs/nextjs/reference/hooks/use-session.md) or [`useAuth()`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) hooks.
...
The `/update-user-metadata` API route uses Clerk's [`updateUserMetadata()`](https://clerk.com/docs/reference/backend/user/update-user-metadata.md) method from the [`JS Backend SDK`](https://clerk.com/docs/js-backend/getting-started/quickstart.md) to update the user's birthday metadata.
...
The `user.reload()` method refreshes the current user's `User` object and their session token. It can be accessed from the [`useUser()`](https://clerk.com/docs/nextjs/reference/hooks/use-user.md) hook.
