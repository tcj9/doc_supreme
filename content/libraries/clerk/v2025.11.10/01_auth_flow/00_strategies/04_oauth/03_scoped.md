...
- **Client**: The third-party app that wants to access a user's data from your application (the resource service). This app is configured to obtain OAuth tokens from Clerk.
...
- **Resource service (or Service Provider, SP)**: The service that holds the data that the client wants to access. In this example, this would be **your own application** that uses Clerk for authentication and route protection.

> If you would like to know more about the terminology around OAuth, check out the [OAuth terminology guide](https://clerk.com/docs/guides/configure/auth-strategies/oauth/overview.md#key-terminology).
...
1. ### Create a Clerk OAuth application
   ...
   3. Complete the following fields:
      ...
      - `Scopes` - The scopes that you would like to leverage. [Learn more about the scopes currently supported by Clerk](https://clerk.com/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth.md#scopes).
...
3. ### Finished 🎉
   ...
   >
   > By default, the consent screen is shown for all newly created OAuth apps, but this can be disabled in each app's settings [in the Clerk Dashboard](https://dashboard.clerk.com/~/oauth-applications), although not recommended. [Learn more about how Clerk's OAuth consent screen works](https://clerk.com/docs/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth.md#consent-screen-management).
