...
`useAuth()` provides authentication data in Client Components. Due to Next.js's default behavior, these components will be statically rendered. If that's fine with you, see the [`reference docs`](https://clerk.com/docs/nextjs/reference/hooks/use-auth.md) for more information, like code examples.
...
The following example demonstrates how to use `<ClerkProvider dynamic>` in a page's layout in order to opt it into dynamic rendering. In this case, the [`root layout`](https://clerk.com/docs/nextjs/getting-started/quickstart.md#add-clerk-provider-to-your-app) would still be wrapped with `<ClerkProvider>` to provide the authentication context to the entire app, but without the `dynamic` prop being passed as to not opt your entire app into dynamic rendering.
