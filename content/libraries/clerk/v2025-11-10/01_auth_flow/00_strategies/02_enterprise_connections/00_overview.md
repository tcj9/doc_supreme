...
2. Select the SAML connection you want to allow subdomains for.
3. Select the **Advanced** tab.
4. Enable or disable the **Allow subdomains** option.
5. Select **Save**.
...
Clerk supports Enterprise SSO via the OpenID Connect (OIDC) protocol, either through [EASIE](#easie) or by [integrating with any OIDC-compatible provider](https://clerk.com/docs/guides/configure/auth-strategies/enterprise-connections/oidc/custom-provider.md).
...
- [Google](https://clerk.com/docs/guides/configure/auth-strategies/enterprise-connections/easie/google.md)
- [Microsoft](https://clerk.com/docs/guides/configure/auth-strategies/enterprise-connections/easie/microsoft.md)
...
It is ultimately the app's responsibility to handle this unauthenticated state and display something appropriate to the user. For example, Next.js apps using [`auth.protect()`](https://clerk.com/docs/reference/nextjs/app-router/auth.md#auth-protect) will automatically redirect the user to the sign-in page.
...
1. In the Clerk Dashboard, navigate to the [**SSO connections**](https://dashboard.clerk.com/~/user-authentication/sso-connections) page.
2. Select the enterprise connection you want to allow multiple identifiers for.
3. Select the **Advanced** tab.
4. Enable the **Allow additional identifiers** option.
5. Select **Save**.
...
The users will not be deleted, so your app will not break. However, they will need to use another [sign-in option](https://clerk.com/docs/guides/configure/auth-strategies/sign-up-sign-in-options.md) such as password or email codes to authenticate themselves moving forward.

### Does Clerk support IdP-initiated SSO?

Yes, for SAML only. Clerk supports both Service Provider-initiated (SP-initiated) and Identity Provider-initiated (IdP-initiated) SSO flows. For more information, see the [authentication flows guide](https://clerk.com/docs/guides/configure/auth-strategies/enterprise-connections/authentication-flows.md).

### How much does it cost?

For development instances, Enterprise connections are always free but limited to a maximum of 25.

Production instances require the Pro or Business plan. Please see [pricing](https://clerk.com/pricing){{ target: '_blank' }} for more information.
