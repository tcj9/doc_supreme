...
Unlike React running inside the browsers, React Native has a very different user experience. For example there is no âtab focusâ, switching from the background to the app is considered as a âfocusâ instead.
To customize these behaviors, you can replace the default browser `focus` and `online` events listeners with React Nativeâs app state detection and other native ported API, and configure SWR to use them.
