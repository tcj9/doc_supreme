...
<Callout emoji="â">
  Please update to the latest version (â¥ 0.3.0) to use this API. The previous <code>useSWRPages</code> API is now deprecated.
</Callout>
...
function App () {
  ...
  const list = []
  for (let i = 0; i < cnt; i++) {
    // ð¨ This is wrong! Commonly, you can't use hooks inside a loop.
