...
when the user clicks the âLogoutâ button:
...
Also, this hook doesnât share states with other `useSWRMutation` hooks.
...
feel faster â no need to wait for the remote source of data.
...
When you have `optimisticData` set, itâs possible that the optimistic data gets
...
When combined with `optimisticData` and `rollbackOnError`, youâll get a perfect optimistic UI experience.
...
```jsx
// â matching array key
mutate((key) => key[0].startsWith('/api'), data)
// â matching string key
mutate((key) => typeof key === 'string' && key.startsWith('/api'), data)

// â ERROR: mutate uncertain keys (array or string)
