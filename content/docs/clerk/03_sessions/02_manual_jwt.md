...
The [`authenticateRequest()`](https://clerk.com/docs/reference/backend/authenticate-request.md) method from the JS Backend SDK accepts the `request` object and authenticates the session token in it.
...
2. ### Get your instance's public key
   ...
   1. Use the Backend API in JSON Web Key Set (JWKS) format at the following endpoint [https://api.clerk.com/v1/jwks](https://clerk.com/docs/reference/backend-api/tag/jwks/get/jwks.md){{ target: '_blank' }}.
