...
title: Responses API
product: vercel
url: /docs/ai-gateway/sdks-and-apis/openai-compat/responses
type: integration
prerequisites:
...
AI Gateway supports the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses), letting you use the official OpenAI SDKs with any provider supported by the gateway. Point your client to the AI Gateway base URL and use `provider/model` identifiers to route requests to OpenAI, Anthropic, Google, and more.
...
| `reasoning`              | object           | Reasoning config with `effort` (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`). Optionally include `summary` to receive a text summary of the model's reasoning |
...
| `prompt_cache_retention` | string           | Retention duration for cached prompts: `in_memory` or `24h`                                                                                                           |
